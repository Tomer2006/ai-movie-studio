...
- **Subagents** — `@screenwriter` (dialogue/narration), `@shotboard` (shot prompts/durations), `@qc` (post-render fixes). Config: `[.opencode/agents/](.opencode/agents/)` and `[opencode.json](opencode.json)`.
- Full workflow and QC loop: `[.opencode/skills/movie-studio/SKILL.md](.opencode/skills/movie-studio/SKILL.md)`.
