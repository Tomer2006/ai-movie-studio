...
- **Required:** a top-level `video_style` object with a non-empty `aesthetic` (whole-film bucket: photoreal live-action vs anime vs documentary, etc.), plus other `video_style` fields as needed for camera, lighting, film texture, grading, pacing. Schema rejects a missing `video_style` or empty `aesthetic`. The pipeline appends it to **every** render prompt. Use `video_style.negative_prompt` for look-wide bans.
