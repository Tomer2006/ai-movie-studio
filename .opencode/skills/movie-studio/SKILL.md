...
3. **Continuity bible** — Edit `continuity_bible.json`; every character in `characters[]` must include a `profile` with locked visual/personality details (`physical_signature`, `face`, `hair`, `eyes`, `body`, `movement`, `personality`, `voice`, `consistency_notes`, `negative_prompt`, and a stable `seed` when possible). **Required** top-level `video_style` with non-empty `aesthetic` (master rendering bucket for the **whole film** — photoreal vs anime vs documentary, etc.), plus look, camera, lighting, texture, grading, pacing as needed; merged into **every** shot render automatically (`python -m studio plan` fails without it).
