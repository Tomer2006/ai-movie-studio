...
- Cost/provider warnings are status information, not permission to downscope. With `VIDEO_PROVIDER=mock`, still preserve the full requested `scenes.json`; mock only changes render quality.
...
8. **Mock path** — If the provider resolves to `mock`, skip automatic visual QC and use `python -m studio render-all` as a full-length pipeline validation run.
